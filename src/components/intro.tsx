...
const SocialLink = ({ href, icon: Icon, tooltip }: { href: string; icon: React.ElementType; tooltip: string }) => (
...
const Intro = () => {
	...
	return (
		...
		>
			...
			<motion.h1 className="text-4xl md:text-6xl font-bold tracking-tighter !leading-tight" variants={itemVariants}>
				Stefano Valenti
			</motion.h1>

			<motion.div variants={itemVariants} className="flex flex-wrap justify-center items-center gap-2 md:gap-3 my-6">
				{roles.map((role) => (
					<span
						key={role}
			...
			<motion.div className="flex flex-col sm:flex-row items-center justify-center gap-4" variants={itemVariants}>
				...
				>
					Resume <HiDownload className="opacity-70 group-hover:translate-y-0.5 transition" />
				</ActionButton>
