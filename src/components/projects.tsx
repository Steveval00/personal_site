...
export default function Projects() {
	...
	const handleToggleShow = () => {
		setProjectsToShow((prevCount) =>
			prevCount === initialProjectsToShow ? filteredProjects.length : initialProjectsToShow
		);
	};
	return (
		<section ref={ref} id="projects" className="hidden scroll-mt-28 mb-28 flex flex-col items-center">
			...
			<div className="flex flex-row justify-center items-center gap-4 my-8">
				...
				<button className={`filter-btn ${filter === "Web" ? "active" : ""}`} onClick={() => handleFilterChange("Web")}>
					Web
				</button>
				<button
